...
import type { DriveFile, ImageSlot, ThemeAlignH, ThemeAlignV } from '../../types/asset';
import { driveImageToDataUrl, enlargeThumbnailUrl } from '../../services/driveApi';

const ALIGN_H_OPTIONS: { value: ThemeAlignH; label: string }[] = [
  { value: 'left',   label: 'Left'   },
  { value: 'center', label: 'Center' },
  { value: 'right',  label: 'Right'  },
];
const ALIGN_V_OPTIONS: { value: ThemeAlignV; label: string }[] = [
  { value: 'top',    label: 'Top'    },
  { value: 'center', label: 'Center' },
...
interface Props {
  ...
  alignH?: ThemeAlignH;
  alignV?: ThemeAlignV;
  onAlignHChange?: (h: ThemeAlignH) => void;
  onAlignVChange?: (v: ThemeAlignV) => void;
  footer?: ReactNode;
}

export function ImageGallery({ label, files, loading, error, currentSlot, onSelect, selectedColor, onColorChange, filterTags, searchable, alignH, alignV, onAlignHChange, onAlignVChange, footer }: Props) {
    ...
    <div className="rounded-lg border border-gray-200 bg-white">
      ...
      {open && (
        <div className="border-t border-gray-100 px-4 pb-4 pt-3 space-y-3">
          ...
          {/* Alignment selector */}
          {alignH !== undefined && alignV !== undefined && onAlignHChange !== undefined && onAlignVChange !== undefined && (
            <div className="space-y-2">
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Alignment</p>
              <div className="grid grid-cols-3 gap-1.5">
                {ALIGN_H_OPTIONS.map(({ value, label }) => (
                  <button
                    key={value}
                    onClick={() => onAlignHChange(value)}
                    className={`rounded-lg border-2 py-1.5 text-xs font-medium transition-all ${
                      alignH === value
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {ALIGN_V_OPTIONS.map(({ value, label }) => (
                  <button
                    key={value}
                    onClick={() => onAlignVChange(value)}
                    className={`rounded-lg border-2 py-1.5 text-xs font-medium transition-all ${
                      alignV === value
                        ? 'border-indigo-500 bg-indigo-50 text-indigo-700'
                        : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'
